...
def count_sequences(line, player):
    ...
    Counts only sequences that can be extended to a winning sequence by adding a cell on either side.
    ...
    :return: A tuple (count_3, count_4) where count_3 is the number of sequences of 3 consecutive cells,
    ...
    if len(line) > 4:
        for i in range(len(line) - 3):
            if line[i:i + 4] == [player] * 4:
                if i != 0 and line[i - 1] == 0:
                    count_4 += 1
                if i + 5 != len(line) and line[i + 5] == 0:
                    count_4 += 1
    if len(line) > 3:
        for i in range(len(line) - 2):
            if line[i:i + 3] == [player] * 3:
                if i != 0 and line[i - 1] == 0:
                    count_3 += 1
                if i + 4 != len(line) and line[i + 4] == 0:
                    count_3 += 1
...
def calculate_score(grid, row, col, player):
   ...
   """
    ...
    for d in range(-row + 1, col):
        ...
        diag2 = [grid[r][d + r] for r in range(max(0, -d), min(row, col - d))]
